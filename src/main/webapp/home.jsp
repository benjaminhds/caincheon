...
$(document).ready( function() {
	...
	// 자동이동
	setTimeout(function() { 
		$("#gotoscrollformouse").trigger("click"); 
		}, 3000);
...
<body>
	<form name="form01" action="/news/alb_view.do" method="POST">
		...
		<div id="wrap">
			<!-- visual -->
			<div class="visual section fp-scrollable">
				<span class="logo"><img alt="천주교인천교구DIOCESE OF INCHEON"
					src="/img/main/visual_logo.png"></span>
				<div class="vis">
					<i>오늘의소식</i>
					<!-- 
                <c:if test = "${fn:length(todayContents.TALK)>0}">
                	<em>${todayContents.TALK}</em> <span>${todayContents.H_DATE}</span>
                </c:if>
                 -->
					<a href="http://www.cbck.or.kr/bbs/bbs_read.asp?board_id=k1200&bid=13013418" target="_blank"><em>
					<!--span lang="zh-CN" style="font:12pt MingLiU;"></span-->&nbsp;2018년 7,8월 피정 목록(CBCK홈페이지 참조)</em></a>
				</div>
				<em class="mouse"><img id="gotoscrollformouse" alt="스크롤하세요"
					src="/img/main/top_mouse.png"><span><img alt=""
						src="/img/main/top_arr.png"></span></em>
				<div class="botBar">
					<a href="#"><em>${todayContents.H_DATE}</em>
						${todayContents.DESCRIPTION}</a>
				</div>
			</div>
			<!-- //visual -->
			<!-- secWrap  -->
			<div class="secWrap section fp-auto-height fp-scrollable" id="mainSection">
				<!-- secs  -->
				<div class="secs">
					<!-- sec01 -->
					<section class="sec01">
						...
						<div>
							<article class="remeber">
								<h4>이달의 사제</h4>
								<i class="moOnly more"><a href="/father/father_cal.do">more<img
										alt="더보기" src="/img/main/ico/more.png"></a></i>
								<ul>
									<c:choose>
										<c:when test="${fn:length(fatherList) > 0}">
											<c:forEach items="${fatherList}" var="fatherList">
												<li><a
													href="/father/father_view.do?p_idx=${fatherList.P_IDX}"><span><em>${fatherList.NAME}</em>
															<span>${fatherList.CHRISTIAN_NAME}</span> <span>${fatherList.PHRASE}</span></span>
														<i><img alt=""
															src="/upload/father/${fatherList.P_IDX}.png"></i> <em><img
															alt="" src="/img/dimmed_50.png"></em></a></li>
											</c:forEach>
										</c:when>
									</c:choose>
								</ul>
							...
							<article class="news">
								...
								<div class="box">
									<div class="left">
										<h5>
											<i><img alt="" src="/img/main/ico/ca.png"></i> <span><em>천주교 인천교구 소식</em> <span>DIOCESE OF INCHEON</span></span>
										</h5>
										...
										<div>
											...
											<ul class="sche on" id="gyogu_monthly_schedule">
												...
												<c:forEach items="${schList}" var="schList">
													<li><em>${schList.GUBUN}</em><a
														href="javascript: viewSchedule('${schList.GUBUN}','${schList.B_IDX}','${schList.S_IDX}')">${schList.TITLE}</a><span>${schList.START_DATE}</span>
													</li>
												</c:forEach>
												</c:if>
											</ul>
	                    ...
	                    <div class="slideWrap">
	                        <figure>
	                            ...
	                            <span class="slides" style="width: 515%; position: relative; transition-duration: 0s; transform: translate3d(-1297.5px, 0px, 0px);">
	                                ...
	                                <c:forEach items="${parList}" var="plist" varStatus="status">
	                                
	                                <c:set var="par_view_uri" value="/gyogu/par_view.do?pageNo=1&b_idx=${plist.B_IDX}&bl_idx=${plist.BL_IDX}" />
	                                
	                                <em <c:if test="${status.last or status.first }">class="bx-clone"</c:if> style="vertical-align:top">
	                                	<c:if test="${fn:length(plist.STRFILENAME1) > 0}">
                                		<i><img src="${plist.FILEPATH1}thumbnail/${plist.STRFILENAME1}" width="335" onClick="javascript: location.href='${par_view_uri}'; "></i>
	                                	</c:if>
	                                	<span class="center" onClick="javascript: location.href='${par_view_uri}'; ">
		                                	<span class="ttl">${plist.TITLE}</span> 
		                                	<c:if test="${plist.CONTENT ne ''}"><span class="txt">${plist.CONTENT}</span></c:if>
	                                	</span>
	                                </em>
					...
					<section class="sec03">
						...
						<div>
							<ul>
								...
								<li><a href="/community/cana_manual.do"><em><img
											alt="" src="/img/main/sec_03_ico_over.png"></em><i><img
							...
							<ol>
								...
								<li><a href="#none">부서</a>
									<ul>
										<li>
											<ol>
												<li><a href="http://youth.caincheon.or.kr/" target="_blank" title="새창">청소년교육국</a></li>
