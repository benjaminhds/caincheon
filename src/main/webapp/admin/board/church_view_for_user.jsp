...
<%@ include file="/admin/_common/inc/head_for_useradmin.jsp" %>
<%
if(sessionMemId==null||sessionMemId.equals("")) {
							...
							<table class="table tbl-form">
								...
								<tbody>
									<tr>
										...
										<td>
											<label for=""><%=sessionMemNm %> </label>
										</td>
									</tr>	
									<!-- <tr>
										<th><label for="">분류</label></th>
										<td>
											<select class="form-control" name="b_idx" id="b_idx">
												<option value="">선택</option>
												<option value="11" <c:if test="${CONTENTS.B_IDX=='11' }" >selected</c:if>>공지</option>
												<option value="17" <c:if test="${CONTENTS.B_IDX=='17' }" >selected</c:if>>소식</option>
											</select>
										</td>
									</tr> -->
									<tr>
										<th><label for="">소속(본당)</label></th>
										<td>${CONTENTS.CHURCH_IDX }
