...
function viewList() {
	document.form01.action = '/myinfo/doctrine_list.do';
...
function modify_contents() {
	document.form02.action = '/myinfo/doctrine_modify.do';
	//document.getElementById('idx').value=idx;
	document.form02.submit();
	return false;
}

function delete_contents(no) {
	document.form01.action = '/myinfo/doctrine_delete.do';
...
<body>
<form name="form01" action="/myinfo/doctrine_insert.do" method="POST">
	<input type="hidden" name="pageNo" id="pageNo" value="${paging.pageNo}" />	
	<input type="hidden" name="doctrine_no" id="doctrine_no" value="${doctrine_no}" />	
	<input type="hidden" name="id" id="id" value="${id}" />	
</form>	

<form name="form02" action="/myinfo/doctrine_modify.do" accept-charset="utf-8" method="POST">
...
<td>
	<c:choose>
		<c:when test="${docContents.APPROVE_YN eq 'Y'}" >
...
<button id="btnDelete" onclick="delete_contents('${docContents.DOCTRINE_NO}');">삭제</button>
</body>
</html>