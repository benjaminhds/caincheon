										...
										<c:if test="${bd_content.USEYN_ATTACHEMENT eq 'Y'}">
											<span class="file">
												<c:if test="${not empty attachList}">
													<em>첨부파일 : </em>
													<c:set var="HEAD_CONST__attach_file_form__MAX_FILE_COUNT" value="${fn:length(attachList)}" />
...
<script type="text/javascript">
	window.onload = function() {
		...
		var bidx	=	$("input[name=i_sBidx]", "#form02").val();
		
		// bishop --> upload/gyogu_album/
		CKEDITOR.replace( 'contents', {
			height: 450,
			filebrowserUploadUrl: '/admin/ckeditor/fileUploadInContentNew_'+bidx+'.do'
	  	});
		
		viewButtonEvent();
		var commentYn	=	"${bd_content.USEYN_COMMENT eq 'Y'}";
