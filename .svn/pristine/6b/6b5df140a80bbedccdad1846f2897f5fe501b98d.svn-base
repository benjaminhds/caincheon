...
function viewList() {
	document.form02.action = '/admin/board/oldAlb_list.do';
	document.form02.submit();
	return false;
}

function check_contents() {
	if( $("#c_idx").val() == "") { alert("구분을 선택해 주세요."); $("#c_idx").focus(); return false; }
...
function insert_contents() {
	if(!check_contents()) { return; }

	document.form01.action = '/admin/board/oldAlb_insert.do';
...
function modify_contents() {
	...
	document.form01.action = '/admin/board/oldAlb_modify.do';
	document.form01.submit();
	return false;
}

function delete_contents(bl_idx) {
		...
		<div id="page-wrapper">
			<div class="container-fluid">
				<div class="row">
					<div class="col-lg-12">
					<c:choose>
					<c:when test="${_params.q_type == 'insert'}">
						<h3 class="page-header">역대교구장 앨범 등록</h3>
					</c:when>
					<c:otherwise>
						<h3 class="page-header">역대교구장 앨범 수정</h3>
					</c:otherwise>
					</c:choose>
					</div>
					<!-- /.col-lg-12 -->
				...
				<!-- /.row -->
<form id="form01" name="form01" enctype="multipart/form-data" method="POST" action="/admin/board/oldAlb_list.do">
 ...
 <input type="hidden" name="filename" id="filename" value=""/>
				...
				<div class="page-form">
							...
							<table class="table tbl-form">
								...
								<tbody>
									<tr>
										...
										<td><select class="form-control" name="c_idx" id="c_idx">
												<option value="">선택</option>
												<option value="1" <c:if test="${CONTENTS.C_IDX == '1'}">selected</c:if> >제1대 교구장</option>
												<option value="2" <c:if test="${CONTENTS.C_IDX == '2'}">selected</c:if> >제2대 교구장</option>
											</select>
										</td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td>
												...
												<c:if test="${fn:length(CONTENTS.STRFILENAME1) > 0 }">
													...
													<p><img src="${CONTENTS.FILEPATH1}thumbnail/${CONTENTS.STRFILENAME1}" /></p>
					...
					<c:otherwise>
						...
						<button type="button" id="btnDelete" class="btn btn-danger btn-lg" onclick="javascript: delete_contents('${CONTENTS.BL_IDX}');return false;"><i class="fa fa-times"></i>삭제</button>
					</c:otherwise>
					</c:choose>
					</p>
					<!-- Buttons : End //-->
...
<form id="form02" name="form02" method="POST" action="/admin/board/oldAlb_list.do">
...
window.onload = function() {
	...
	CKEDITOR.replace( 'contents', {
    	filebrowserUploadUrl: '/admin/ckeditor/fileUploadInContent05.do'//filebrowserUploadUrl: '/admin/board/boardImageUploadBoard06.do'
  	});
	// thumbnail
