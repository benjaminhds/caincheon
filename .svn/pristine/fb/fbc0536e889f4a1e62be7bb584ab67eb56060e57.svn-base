...
function viewList() {
	document.form01.action = '/myinfo/eucha_list.do';
...
function modify_contents() {
	...
	document.form02.action = '/myinfo/eucha_modify.do';
	//document.getElementById('idx').value=idx;
...
function delete_contents() {
	document.form02.action = '/myinfo/eucha_delete.do';
...
<body>
<form name="form01" action="/myinfo/eucha_insert.do" method="POST">
	<input type="hidden" name="pageNo" id="pageNo" value="${paging.pageNo}" />	
	<input type="hidden" name="eucha_mid" id="eucha_mid" value="${eucha_mid}" />	
	<input type="hidden" name="id" id="id" value="${id}" />	
</form>	

<form name="form02" action="/myinfo/eucha_modify.do" accept-charset="utf-8" method="POST">
	<input type="hidden" name="pageNo" id="pageNo" value="${paging.pageNo}" />	
...
<td>
	<c:choose>
		<c:when test="${euchaContents.APPROVE_YN eq 'Y'}" >
...
<td>
	<c:choose>
		<c:when test="${euchaContents.APPLY_TYPE eq '1'}" >
...
<td>강좌신청날짜</td>
<td>
	<input type="date" name="lecture_apply_day" id="lecture_apply_day" value="${euchaContents.LECTURE_APPLY_DAY}">
</tr>

<tr>
<td>성명</td>
<td>
	<input  type="text" name="man_name" id="man_name"  value="${euchaContents.MAN_NAME }"/>
	<input  type="text" name="woman_name" id="woman_name"  value="${euchaContents.WOMAN_NAME }"/>
</td>	
</tr>
