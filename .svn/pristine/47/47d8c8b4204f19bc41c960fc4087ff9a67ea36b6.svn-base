...
function goSearch() {
 	if(document.getElementById('searchText').value != '' && document.getElementById('searchGubun1').value == '') {
		alert('검색 조건이 선택되어야 합니다.');
		document.getElementById('searchGubun1').focus();
		return false;
	}
	document.form01.submit();
    return false;
...
function insertContents() {
	document.form01.action = '/admin/board/cure_view.do';
	document.getElementById('query_type').value = "insert";
	document.form01.submit();
    return false;
}

function modifyContents(c_idx, bl_idx) {
	document.form01.action = '/admin/board/cure_view.do';
	document.getElementById('query_type').value = "modify";
	$("#c_idx").val(c_idx).prop("selected", true);
	document.getElementById('bl_idx').value=bl_idx;
	document.form01.submit();
    return false;
}

function deleteContents(c_idx, bl_idx) {
	if (confirm("정말 삭제하시겠습니까??") == true){    //확인
		document.form01.action = '/admin/board/cure_delete.do';
		$("#c_idx").val(c_idx).prop("selected", true);
		document.getElementById('bl_idx').value=bl_idx;
		document.form01.submit();
	}
...
<body>
<form class="form-inline" name="form01" action="/admin/board/cure_list.do" method="POST">
...
<input type="hidden" name="bl_idx" id="bl_idx" value="${bl_idx}"/>
		...
		<div id="page-wrapper">
			<div class="container-fluid">
				<div class="row">
					<div class="col-lg-12">
						<h3 class="page-header">사목자료 목록</h3>
                	</div>
                </div>
                <!-- /.col-lg-12 -->
            </div>
								...
								<dd>
									<div class="group">
										<select class="form-control form-control-short w-200 mr-10" name="c_idx" id="c_idx">
										  ...
										  <!-- c_idx -->
										  <option value="5" <c:out value="${schTextGubunMap1['5']}"/>>전례</option>
										  <option value="6" <c:out value="${schTextGubunMap1['6']}"/>>양식</option>
										  <option value="7" <c:out value="${schTextGubunMap1['7']}"/>>교리</option>
										  <option value="8" <c:out value="${schTextGubunMap1['8']}"/>>사목</option>
										  <option value="9" <c:out value="${schTextGubunMap1['9']}"/>>선교</option>
										</select>
									</div>
								</dd>
									...
									<div class="row">
										<div class="group">
											<select class="form-control form-control-short w-200 mr-10" name="searchGubun1" id="searchGubun1">
											  ...
											  <option value="all" <c:out value="${schTextGubunMap2['title2']}"/>>제목+내용</option>
							...
							<table class="table table-striped table-bordered table-hover">
                                ...
                                <c:choose>
	                                ...
	                                <tr <c:if test="${list.IS_VIEW eq 'N'}">style="color: darkgray;"</c:if> >
	                                    ...
	                                    <td class="center"><strong><font style='<c:if test="${list.IS_VIEW eq 'N'}">color:darkgray</c:if>' />${list.CATEGORY_NAME}</font></strong></td>
	                                    <td>${list.TITLE}</td>
	                                    <td class="center">${list.WRITER}</td>
	                                    <td class="center">${list.REGDATE}</td>
	                                    <td class="center">${list.HIT}</td>
	                                    <td class="center"><button name="btnEdit" type="button" class="btn40 btn-success btn-circle" title="Edit" onClick="javascript:modifyContents('${list.C_IDX}','${list.BL_IDX}')"><i class="fa fa-pencil"></i></button>
	                                    </td>
	                                    <td class="center"><button name="btnDel" type="button" class="btn40 btn-danger btn-circle" title="Delete" onClick="javascript:deleteContents('${list.C_IDX}','${list.BL_IDX}')"><i class="fa fa-times"></i></button>
	                                    </td>
	                                </tr>
	                                </c:forEach>
									</c:when>
